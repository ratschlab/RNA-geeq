...
def parse_options(argv):
    ...
    filter = OptionGroup(parser, 'INPUT FILTER SETTINGS')
    filter.add_option('-M', '--max_intronsn', dest='max_intron_len', metavar='INT', type='int', help='maximal intron length [100000000]', default='100000000')
    filter.add_option('-e', '--min_exon_len', dest='min_exon_len', metavar='INT', type='int', help='minimal exon length [0]', default=0)
    filter.add_option('-X', '--max_mismatches', dest='max_mismatches', metavar='INT', type='int', help='maximum number of allowed mismathes [10000]', default=10000)
    filter.add_option('-m', '--min_read_coverage', dest='min_coverage', metavar='INT', type='int', help='minimal coverage to count an intron [1]', default='1')
    filter.add_option('-R', '--ignore_reads', dest='exclude_set', metavar='FILE', help='file containing the reads to ignore', default='-')
    filter.add_option('-x', '--exclude_chrms', dest='exclude_chrm', metavar='STRINGLIST', help='list of comma separated chromosomes to exclude from evaluation', default='-')
    anno = OptionGroup(parser, 'ANNOTATION OPTIONS')
    anno.add_option('-S', '--show_sources', dest='show_sources', action='store_true', help='only show available sources of gff file', default=False)
    anno.add_option('-L', '--sources', dest='sources', help='list of comma-separated soources to use from annotation', default='-')
